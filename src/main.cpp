...
#if defined(NDEBUG)
#error "barandos cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // POW Year 0
    if (nHeight == 0) {
        nSubsidy = 100000.00 * COIN;
    } else if (nHeight <= 30000 && nHeight >= 1) {
	    nSubsidy = 20 * COIN;
    } else if (nHeight <= 100000 && nHeight >= 30001) { // Keep reward schedule.
        nSubsidy = 10 * COIN;
...
int64_t GetSplitReward(int64_t blockValue, int64_t nHeight) {
	int64_t ret = 0;
	if (nHeight == 0) {
        ret = 0;
    } else if (nHeight <= 300 && nHeight >= 1) {
	    ret = 0;
    } else if (nHeight <= 30000 && nHeight >= 301) { 
        ret = blockValue / 100 * 20;
	} else if (nHeight <= 100000 && nHeight >= 30001) { 
...
{
    return GetSplitReward(blockValue, nHeight);
}
...
{
    RenameThread("barandos-scriptch");
