...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("barandosMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("barandosMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("barandosMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("barandos-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running barandosMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
